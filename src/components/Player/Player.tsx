...
function Player({ currentTrack, allTracks, onTrackEnd, onTrackChange }: PlayerProps) {
  ...
  const fullPlayerRef = useRef<BottomSheetModal | null>(null);
  const canOpenFullPlayer = useMemo(() => Boolean(hasTrack && !loading), [hasTrack, loading]);
  
  useEffect(() => {
    onTrackChangeRef.current = onTrackChange;
    onTrackEndRef.current = onTrackEnd;
    playbackModeRef.current = playbackMode;
  }, [onTrackChange, onTrackEnd, playbackMode]);
  ...
  useEffect(() => {
    const loadTrack = async () => {
      ...
      // If it's the same track, don't reload it
      ...
      try {
        ...
        const soundInstance = new SoundDriver(audioUrl);
  ...
  const handleNextTrack = useCallback(() => {
    ...
    if (playbackMode === 'shuffle') {
      ...
      if (allTracks.length === 1) {
        ...
        const currentIndex = allTracks.findIndex((t: Track) => t.id === currentTrack.id);
        do {
          nextIndex = Math.floor(Math.random() * allTracks.length);
        } while (nextIndex === currentIndex && allTracks.length > 1);
      }
