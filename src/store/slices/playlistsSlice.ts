...
export const DEFAULT_FAVORITES_COVER = 'linear-gradient(135deg,rgb(246, 215, 250) 0%, #f5576c 100%)'
...
const getStoredPlaylists = async (): Promise<Playlist[]> => {
  try {
    const raw = await AsyncStorage.getItem(PLAYLISTS_STORAGE_KEY)
    if (!raw) return []
...
const playlistsSlice = createSlice({
  ...
  initialState,
  reducers: {
    initializePlaylists: (state, action: PayloadAction<Playlist[]>) => {
      state.playlists = [
        {
          id: FAVORITES_PLAYLIST_ID,
          name: 'Favorite',
    ...
    setPlaylists: (state, action: PayloadAction<Playlist[]>) => {
      ...
      AsyncStorage.setItem(PLAYLISTS_STORAGE_KEY, JSON.stringify(customPlaylists))
    },
