...
const getStoredFavoriteTracks = async (): Promise<PlaylistTrack[]> => {
  try {
...
const favoritesSlice = createSlice({
  ...
  reducers: {
    setFavoriteTracks: (state, action: PayloadAction<PlaylistTrack[]>) => {
      ...
      AsyncStorage.setItem(FAVORITES_STORAGE_KEY, JSON.stringify(action.payload))
    },
    initializeFavorites: (state, action: PayloadAction<PlaylistTrack[]>) => {
      state.favoriteTracks = action.payload
    ...
    addToFavorites: (state, action: PayloadAction<PlaylistTrack>) => {
      ...
      if (!trackExists) {
        ...
        AsyncStorage.setItem(FAVORITES_STORAGE_KEY, JSON.stringify(state.favoriteTracks))
      }
    },
...
export const { setFavoriteTracks, initializeFavorites, addToFavorites, removeFromFavorites } = favoritesSlice.actions
